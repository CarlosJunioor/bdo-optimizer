...
impl App {
    ...
    fn start_capture(&mut self, ctx: egui::Context) {
        ...
        let params = CaptureParams {
            ...
            // Recorded by `refresh_verification` in the same step that read the
            // mask, rather than by a second scan here — between the two the
            // verified instance could exit and a different one appear, and the
            // capture would then be saved under the wrong provenance.
            verified_pid: self.optimize.verified_pid,
        };
